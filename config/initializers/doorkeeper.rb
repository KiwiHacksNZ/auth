...
Doorkeeper.configure do
  ...
  application_class "::Program"
  access_token_class "::OAuthToken"

  access_token_expires_in 6.months
  # Explicitly set table names to use our custom tables
  # This ensures that even if the default Doorkeeper classes are referenced,
  # they point to the correct tables
  #
  # Set table names for Doorkeeper's default classes
  Doorkeeper::Application.table_name = "programs" if defined?(Doorkeeper::Application)
  ...
  admin_authenticator do
    ...
    cu = Backend::User.find_by(id: session[:user_id])
    if cu
      head :forbidden unless cu.super_admin?
    else
  ...
  optional_scopes :openid, :profile, :email, :phone, :birthdate, :slack_id, :verification_status, :legal_name, :basic_info
